...
public function sla_op($Naam,$Genre,$Actors,$Tijd,$Plot,$Poster,$Date,$QR){
...
public function Delete_film($ID){
	$result = $this->db->query("DELETE FROM films WHERE Id = '$ID'");
	return $result;
}

public function update_trailer_omschrijving($id,$link,$Plot){
	$result = $this->db->query("UPDATE films SET Youtube='$link', Plot='$Plot' WHERE Id='$id'");
	return $result;
}

public function update_omschrijving($id,$Plot){
	$result = $this->db->query("UPDATE films SET Plot='$Plot' WHERE Id='$id'");
	return $result;

}

public function update_Trailer($id,$Trailer){
	$result = $this->db->query("UPDATE films SET Youtube='$Trailer' WHERE Id='$id'");
	return $result;
